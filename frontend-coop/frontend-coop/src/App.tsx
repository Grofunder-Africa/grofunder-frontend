...
// Curated one-click options, so most cooperatives never need to touch the
// raw picker — just something that reads as "ours" without any design work.
const THEME_PRESETS = [
  { name: 'Grofunder green', hex: '#09AF0F' },
  { name: 'Harvest gold', hex: '#C68A1F' },
  { name: 'Terracotta', hex: '#B5433E' },
  { name: 'Highland blue', hex: '#3D6E96' },
  { name: 'Coffee brown', hex: '#6B4A2F' },
  { name: 'Deep plum', hex: '#6B3E6B' },
  { name: 'Forest', hex: '#1F6B3E' },
  { name: 'Charcoal', hex: '#3A4038' },
];
...
function Farmers() {
  ...
  useEffect(() => {
    if (editing) {
      ...
      setEditMemberNo(editing.coop_member_no ?? '');
      setEditErr('');
    }
  }, [editing]);
  ...
  async function saveComplete() {
    ...
    try {
      ...
      if (editMemberNo.trim() !== (editing.coop_member_no ?? '')) fields.coopMemberNo = editMemberNo.trim();
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 18 }}>
            {THEME_PRESETS.map((p) => (
              <button
                ...
                title={p.name}
                style={{
                  width: 34, height: 34, borderRadius: 9, background: p.hex, cursor: 'pointer',
                  border: themeColor.toUpperCase() === p.hex ? '2.5px solid var(--ink)' : '0.5px solid var(--line)',
                  padding: 0,
                }}
                aria-label={p.name}
              />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <input
                ...
                style={{ width: 46, height: 46, padding: 0, border: '0.5px solid var(--line)', borderRadius: 10, cursor: 'pointer', background: 'none' }}
                aria-label="Custom cooperative color"
              />
              <div>
